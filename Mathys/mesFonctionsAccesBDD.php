...
function getLibelleVilles(){
    $Lepdo = connexionBDD();
    $sql = "SELECT libelleVille from ville";
    $test=$Lepdo->prepare($sql);
    $test->execute();
    $lesVilles = $test->fetchAll();
    return $lesVilles;
}

function getAllTranche($pdo){
...
function statsVille($pdo,$dateDebut,$dateFin){
    $allVille = getLibelleVilles();
    ...
    foreach ($allVille as $ville) {
        $cmmd->bindValue(':v',$ville['libelleVille']);
        $cmmd->execute();
        $count=$cmmd->fetch();    
        array_push($countVille, $count[0]);
    }
    return($countVille);
}
